...
- **PDF 下载**：支持两种下载方式，自动判断标准可用方式
  ...
  - **预览方式下载**：通过 `c.gb688.cn/bzgk/gb/showGb` 获取预览页面结构，下载图块资源并重组为 PDF（标准仅允许预览时使用）
- **验证码识别**：
...
5. **国标预览方式**：部分国家标准仅允许在线预览，不允许直接下载。爬虫会自动切换为预览方式下载，即获取预览页面结构、下载图块资源、重组页面并生成 PDF。此方式下载的 PDF 质量可能略低于直接下载的原始 PDF。
...
# 输出示例：
# 2025-05-15 10:05:00 [INFO] 待下载标准: 44700 个
