...
- **PDF 下载**：支持两种下载方式
  - `requests` 直接下载（需 c.gb688.cn 可访问）
  - `Playwright` 浏览器下载（处理 JS 渲染和验证码）
...
# 可选：安装 Playwright（国家标准 PDF 下载备选方案）
...
uv run python run_crawler.py gb --download           # 下载国家标准 PDF
...
uv run python scripts/gb_crawler.py --download       # 下载 PDF
...
1. **国家标准 PDF 下载**：国家标准 PDF 下载服务器 `c.gb688.cn` 可能存在网络访问限制，从境外 IP 或部分网络环境下无法直接访问。如遇此问题，建议在大陆网络环境下运行，或使用 Playwright 浏览器方式下载。
...
4. **验证码识别率**：`ddddocr` 对行业标准网站的验证码识别率并非 100%，但通过多次重试机制（默认最多 10 次），可以覆盖绝大多数情况。
...
# ===== 国家标准统计 =====
#   强制性国家标准: 528 条, 已下载 0
#   推荐性国家标准: 43260 条, 已下载 0
#   指导性技术文件: 912 条, 已下载 0
#   国家标准总计: 44700 条, 已下载: 0
#
# ===== 行业标准统计 =====
#   行业分布(Top 10):
#     NB: 3200 条
...
#     not_public: 5000
#     no_file: 200
#   行业标准总计: 85000 条, 已下载: 150
#
# ===== 总计 =====
#   标准: 129700 条, 已下载PDF: 150 个
```
...
- 分页信息通过正则匹配页码元素获取
- PDF 下载优先使用 `requests` 直接请求，失败时降级到 `Playwright` 无头浏览器方式
